...
class AMIOutsourcePlaylist(AmiBase):
    ...
    def _get_context(self, payload: FormRequest):
        ...
        if original_vendor:
            original_vendor = self.sg_session.find_one(
                "Group", [["id", "is", original_vendor["id"]]], ["code"]
            )
...
def run():
    ...
    logger.info(
        f"Starting AMI Create Outsource Playlist service on port {AMI_OUTSOURCE_PLAYLIST_PORT}"
    )
