...
from ami_weekly_status_report.app import AMIWeeklyStatusReport, SubmitRequest, Request
...
class AMIWeeklyStatusReportSweetfoot2(AMIWeeklyStatusReport):
    ...
    def submit(self, request: Request, payload: SubmitRequest):
        ...
        shot_fields.extend(
            ["compositing___sg_blocking_date", "sg_latest_client_version_sent"]
        )
    ...
    def _get_latest_client_version(self, shots):
        ...
        for shot in shots:
            ...
            if not version:
                logger.warning(
                    f"Unable to get sg_latest_client_version_sent from shot {shot['id']}"
                )
                continue
            found = reg.findall(version)
            if not found:
                logger.warning(
                    f"Unable to find version suffix with regex on shot: {shot['id']}"
                )
                continue
    ...
    def _get_shot_compositing_milestone(self, assets, shots):
        ...
        for asset in assets:
            ...
            asset["shot_blocking_compositing"] = shot.get(
                "compositing___sg_blocking_date"
            )
    ...
    def _get_episode(self, assets, shots):
        for asset in assets:
            ...
            linked_shot = self.sg_session.find_one(
                "Shot", [["id", "is", linked_shot["id"]]], ["sg_sequence"]
            )
            if not linked_shot:
                logger.warning(
                    f"Could not find Shot with id {linked_shots[0].get('id')}"
                )
                continue
            ...
            sequence = self.sg_session.find_one(
                "Sequence", [["id", "is", sequence["id"]]], ["episode"]
            )
            if not sequence:
                logger.warning(
                    f"Could not find Sequence with id {linked_shot.get('sg_sequence', {}).get('id')}"
                )
                continue
            ...
            if not episode:
                ...
                continue
            episode = self.sg_session.find_one(
                "Episode", [["id", "is", episode["id"]]], ["code"]
            )
    ...
    def fetch_query_fields(self, entity_type, entities, query_fields):
        ...
        for field in query_fields:
            schema_per_field[field] = self.sg_session.schema_field_read(
                entity_type, field
            )[field]
        ...
        for field, schema in schema_per_field.items():
            ...
            order = (
                schema.get("properties", {}).get("summary_value", {}).get("value", {})
            )
            ...
            for flt in filters:
                ...
                for v in values:
                    if isinstance(v, dict) and v.get("name") in (
                        "Current Shot",
                        "Current Asset",
                    ):
                ...
                if values_final:
                    ...
                    filter_list.append(
                        [flt.get("path"), flt.get("relation"), flt.get("values")]
                    )
    ...
    def convert_field_entities_to_text(self, entities):
        ...
        for entity_type, ids in ids_by_type.items():
            records = self.sg_session.find(
                entity_type, [["id", "in", list(ids)]], ["code"]
            )
    ...
    def get_dates_per_tasks(self, entities, entity_fields):
        ...
        delimiter = "___"
        task_names = list(
            set([x.split(delimiter)[0] for x in entity_fields if delimiter in x])
        )
        task_fields = list(
            set([x.split(delimiter)[-1] for x in entity_fields if delimiter in x])
        )
        fields = task_fields.copy()
        fields.extend(["entity", "content"])
        sg_tasks = self.sg_session.find(
            "Task",
            [
    ...
    def fill_overview(self, shots, assets):
        ...
        friday_of_the_next_week = (current_monday + timedelta(days=7 + 4)).strftime(
            self.date_format
        )
            ...
            for x, sg_status in status_map.items():
                if k.endswith(x):
                    ...
                    if k.startswith("_assets_"):
                        data[k] = len(
                            [y for y in assets if y["sg_status_list"] == sg_status]
                        )

                    elif k.startswith("_shots_"):
                        data[k] = len(
                            [y for y in shots if y["sg_status_list"] == sg_status]
                        )
        ...
        for k, v in data.items():
            ...
            if k.endswith("_percent") and k.startswith("_shots"):
                value = (
                    round(original_value / len(shots) * 100, 2) if len(shots) > 0 else 0
                )
                data[k] = f"{value}%"
            elif k.endswith("_percent") and k.startswith("_assets"):
                value = (
                    round(original_value / len(assets) * 100, 2)
                    if len(assets) > 0
                    else 0
                )
                data[k] = f"{value}%"

        for tag in self.crd_template.tags:
            ...
            self.crd_template.replacements[tag].set_value(str(value))
